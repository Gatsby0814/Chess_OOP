...
public class BoardPanel extends JPanel implements Observer {
    ...
    public void submitMoveRequest(char originFile, int originRank, char destinationFile, int destinationRank) {
        if (getSquarePanel(originFile, originRank).getComponent(0) != null ) {
            ...
            gameModel.onMoveRequest(originFile, originRank, destinationFile, destinationRank);//말 움직이기
        }
    }
    ...
    public void preDrag(char originFile, int originRank, int dragX, int dragY) {
        ...
        if (originPiece != null) {
            ...
            for (char file = 'a'; file < 'i'; file++) {
                for (int j = 1; j < 9; j++) {
                    Move checkCheck = new Move(originPiece, originFile, originRank, file, j);
                    if (checkCheck.getPiece().validateMove(checkCheck) && MoveValidator.validateClearPath(checkCheck)) {
//                        int f = Character.getNumericValue(file) - 1;
//                        int r = j - 1;
                        getSquarePanel(file,j).setBackground(Color.PINK);
                    }
                }
            }
        }
    ...
    public void postDrag() {
        ...
        for (char file = 'a'; file < 'i'; file++) {
            for (int j = 1; j < 9; j++) {
                if (getSquarePanel(file, j).getBackground() == Color.PINK) {
                    int f = Character.getNumericValue(file);
                    int r = j;
                    getSquarePanel(file, j).setBackground(f % 2 == r % 2 ? Color.WHITE : Color.GRAY);
                }
            }
        }
    }
...
//What is BoardReversed? Is it for additional work or ..
...
//import java.util.Observer;
//
//public class BoardPanel extends JPanel implements Observ
// er {
//
//    public static final int SQUARE_DIMENSION = 100;
//
